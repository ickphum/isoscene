...
__PACKAGE__->mk_accessors(qw( canvas palette toolbar scene last_save_name bitmap));
...
#            [ wxACCEL_SHIFT, 'C', wxID_CLEAR ],
    ...
    Wx::Event::EVT_PAINT( $cube_pnl, \&paint_cube );

    my $arrows_btn = Wx::BitmapButton->new($tool_panel, -1, $bitmap->{arrows_off});
    $arrows_btn->SetBitmapHover($bitmap->{arrows_on});
    $arrows_btn->SetBitmapSelected($bitmap->{arrows_on});
    $tool_sizer->Add($arrows_btn, 0, wxALIGN_CENTRE);

    my $area_btn = Wx::BitmapButton->new($tool_panel, -1, $bitmap->{area_left_off});
    $area_btn->SetBitmapHover($bitmap->{area_left_on});
    $area_btn->SetBitmapSelected($bitmap->{area_left_on});
    $tool_sizer->Add($area_btn, 0, wxALIGN_CENTRE);

    my $paintpot_btn = Wx::BitmapButton->new($tool_panel, -1, $bitmap->{paintpot_off});
    $paintpot_btn->SetBitmapHover($bitmap->{paintpot_on});
    $paintpot_btn->SetBitmapSelected($bitmap->{paintpot_on});
    $tool_sizer->Add($paintpot_btn, 0, wxALIGN_CENTRE);

    my $menu_btn = Wx::BitmapButton->new($tool_panel, -1, $bitmap->{menu});
    $tool_sizer->Add($menu_btn, 0, wxALIGN_CENTRE);
